...
android {
  ...
  defaultConfig {
    applicationId = "com.jinn.Voice"
    val today = SimpleDateFormat("yyyyMMdd").format(Date())
    versionCode = today.toInt()
    versionName = today
    buildConfigField(type = "String", name = "BUILD_TIMESTAMP", value = "\"${SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(Date())}\"")
