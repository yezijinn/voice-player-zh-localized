...
class BookPlayViewModelTest {
  ...
  @Test
  fun sleepTimerDialogShowsAndAccepts() = scope.runTest {
    viewModel.toggleSleepTimer()
    yield()
    assertEquals(expected = true, actual = viewModel.showSleepTimerDialog.value)

    viewModel.onAcceptSleepTimer(10)
    yield()
    verify(exactly = 1) {
  ...
  @Test
  fun sleepTimerEpisodeCountdown() = scope.runTest {
    viewModel.toggleSleepTimer()
    yield()
    assertEquals(expected = true, actual = viewModel.showSleepTimerDialog.value)
